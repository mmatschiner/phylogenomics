...
This tutorial requires **BEAST2**, **bModelTest**, **Tracer**, **FigTree**, **ASTRAL**, **PAUP\***, **SNAPP**, <!--**SNAPPER**,--> **babette**, **ape**, and **PhyloNet**. <!--**SpeciesNetwork**,--> All of these tools should be installed at this stage. Details about the installation of these tools can be found in tutorials [Bayesian Phylogenetic Inference](../bayesian_phylogeny_inference/README.md), [Maximum-Likelihood Species-Tree Inference](../ml_species_tree_inference/README.md), [Species-Tree Inference with SNP Data](../species_tree_inference_with_snp_data/README.md), [Divergence-Time Estimation with SNP Data](../divergence_time_estimation_with_snp_data/README.md), [Maximum-Likelihood Inference of Species Networks](../ml_inference_of_species_networks/README.md), <!--[Bayesian Inference of Species Networks](../bayesian_inference_of_species_networks/README.md), -->and [Analysis of Introgression with SNP Data](../analysis_of_introgression_with_snp_data/README.md)<!--, and [Analysis of Introgression with Tree Topologies](../analysis_of_introgression_with_tree_topologies/README.md)-->.

* **Msprime:** The program [Msprime](https://tskit.dev/msprime/docs/stable/intro.html) ([Baumdicker et al. 2021](https://doi.org/10.1093/genetics/iyab229)) is the most modern and actively developed tool for the simulation of genomic data based on the coalescent model. Msprime is available as a Python library that is bundled with binary tools. It is installed on lynx and can be used as described in the tutorial below.


<a name="msprime"></a>
## Coalescent simulations with Msprime

All of the inference methods used in other tutorials of this course (e.g. [Maximum-Likelihood Species-Tree Inference](../ml_species_tree_inference/README.md), [Bayesian Species-Tree Inference](../bayesian_species_tree_inference/README.md), [Species-Tree Inference with SNP Data](../species_tree_inference_with_snp_data/README.md), [Divergence-Time Estimation with SNP Data](../divergence_time_estimation_with_snp_data/README.md), [Bayesian Inference of Species Networks](../bayesian_inference_of_species_networks/README.md), and [Analysis of Introgression with SNP Data](../analysis_of_introgression_with_snp_data/README.md)) are based on assumptions that may in fact rarely be met by empirical datasets. For example, ASTRAL and StarBeast3<!--, and SpeciesNetwork--> assume the absence of recombination within a locus, ASTRAL, SVDQuartets, StarBeast3, and SNAPP<!--, and SNAPPER--> assume the absence of gene flow, and StarBeast3 and SNAPP<!--, and SNAPPER--> assume constant population sizes (this assumption can be relaxed for StarBeast3, but it was used in the [Bayesian Species-Tree Inference](../bayesian_species_tree_inference/README.md) tutorial). And while Dsuite does not implement a model but only report the *D*-statistic for a set of quartets, the interpretation of the *D*-statistic as support for introgression between two species is based on the assumption (among others) that only this pair of species is affected by introgression and that introgression was direct and not flowing through unsampled species.
...
To perform simulations with Msprime, we are going to write a script in Python, and we will execute this script on the command line. The script can be written with any text editor available on lynx.

* Write a new script on lynx that is named `simulate_data.py` and has the following content:

		import msprime
		ts = msprime.sim_ancestry(2)

* Activate the conda environment and install Msprime in it:

		conda_init_shared
		python -m pip install msprime

* Execute the Python script `simulate_data.py`:
...
In order to use the simulated genomic data for inference, we will need to export it in a format that is accepted by the inference methods. Thus, we will need to generate sequence alignments for tools like ASTRAL and StarBeast3<!--, SpeciesNetwork-->, and VCF files for SVDQuartets, SNAPP, <!--SNAPPER,--> and Dsuite. Fortunately, Msprime has a very convenient funtion for export in VCF format, named `write_vcf`, but in order to generate sequence alignments, we will need a separate script.
...
* Retry running the above script after replacing "samples=1," in the `msprime.sim_ancestry` command with "samples={"A": 1, "B": 1},", indicating that we sample a single diploid individual from both populations. This time, the script is likely to finish without errors. If, however, you still see an error message saying "A variant position of 0 was found in the VCF output", you could follow the provided suggestion and add the "allow_position_zero" argument to the "write_vcf" function, by changing the last line of the script to `mts.write_vcf(vcf, allow_position_zero=True)`.
...
* Complete the script by adding commands to simulate ancestry and mutations and to export the data in VCF format:
		...
		# Export genomic data in vcf format.
		with open("simulation.vcf", "w") as vcf:
			mts.write_vcf(vcf, allow_position_zero=True)

	Executing this script should take around 30 minutes, which means that writing a Slurm script will now be more convenient than launching it directly with `python`, as the Slurm script would allow us to run msprime in the background (see next point).
...
* While the above job is running on lynx, we can already prepare another script, with which we simulate genomic data as before, but now with additional introgression between *Neolamprologus marunguensis* (neomar) and *Neolamprologus olivaceous* (neooli), as inferred by [Bouckaert et al. (2019)](https://doi.org/10.1371/journal.pcbi.1006650). To do that, first copy the existing script named `simulate_data.py` to a new file named `simulate_data_introgression1.py`:

		cp simulate_data.py simulate_data_introgression1.py
		
* Open the new file `simulate_data_introgression1.py` in a text editor. To simulate genomic data as before but with introgression between *Neolamprologus marunguensis* (neomar) and *Neolamprologus olivaceous* (neooli), we need to modify the demographic model. We'll use the [`set_migration_rate`](https://tskit.dev/msprime/docs/stable/api.html#msprime.Demography.set_migration_rate) command to specify unidirectional introgression between the two species, from *Neolamprologus marunguensis* (neomar) into *Neolamprologus olivaceous* (neooli), as inferred by [Bouckaert et al. (2019)](https://doi.org/10.1371/journal.pcbi.1006650). If we would instead prefer to specify a symmetric rate of introgression, we could do so using the [`set_symmetric_migration_rate`](https://tskit.dev/msprime/docs/stable/api.html?highlight=set_symmetric_migration_rate#msprime.Demography.set_symmetric_migration_rate) command. More information about both commands can be found in the [Msprime manual](https://tskit.dev/msprime/docs/stable/demography.html).
...
* To execute the new script `simulate_data_introgression1.py` on lynx, a Slurm script is again needed. Copy the first Slurm script to a new file named `simulate_data_introgression1.slurm`:

		cp simulate_data.slurm simulate_data_introgression1.slurm

* Then, open the Slurm script `simulate_data_introgression1.slurm` with Emacs or another text editor, and change the lines specifying the job name (line 4) and the output (line 14) as well as the line on which the script is executed (the last line):
...
* Open file `simulate_data_bottleneck.py` in a text editor, and add a line with the following command, after the command to set up the demographic model:
...
* To execute `simulate_data_bottleneck.py` on lynx, we need a final Slurm script. Prepare this Slurm script by copying `simulate_data.slurm` to a new file named `simulate_data_bottleneck.slurm` and then change the lines specifying the job name (line 4), the output (line 14), and the name of the script to be executed (the last line):
...
* **Question 3:** Actually, while some coalescence times are younger than 200 generations, most are older, indicating that the mean coalescence time in the simulations is older than the expected 200 generations. Repeating the simulation with a much longer sequence length, to produce a very large number of trees in the TreeSequence, shows that the mean coalescence time across trees is around 370 generations. This can be tested with the following script (to run it, the numpy library is required, but should already be installed. If not, this can be done with `python -m pip install numpy`):
	...
	To see the results of this script, the output file `simulation.pdf` needs to be dowloaded from lynx to your local computer and opened with a PDF viewer. The scatter plot in this output file should show that old coalescence times are only found for short regions:<p align="center"><img src="img/matplotlib1.png" alt="matplotlib" width="700"></p>
