...
The entire, now prolific field of introgression studies was launched with the landmark paper presenting the Neanderthal genome. In that paper, [Green et al. (2010)](https://doi.org/10.1126/science.1188021) took the opportunity provided by the Neanderthal genome to test for signals of past genetic exchange with modern humans. The fact that these signals were in fact found changed our understanding of what makes us human, and how our species interacted with others in the past. To identify the traces of genetic exchange in our genomes, Green et al. (2010) developed two statistical methods that still remain among the most commonly used tests for introgression: the *D*-statistic, based on the so-called "ABBA-BABA test", and the related estimate of the admixture fraction *f*, also referred to as the "*f*<sub>4</sub>-ratio". Both are calculated from patterns of allele sharing across populations or closely related species. Although they were developed in a population genetic framework, the methods can be used to learn about hybridization and introgression within groups of closely related species, as long as common population genetic assumptions hold – namely that (a) the species share a substantial amount of genetic variation due to common ancestry and incomplete lineage sorting; (b) recurrent and back mutations at the same sites are negligible; and (c) substitution rates are uniform across species.

When introgression occurs between two species, signals of that gene flow may appear in the *D*-statistic or the *f*<sub>4</sub>-ratio not only when these are calculated for two hybridizing species, but also when close relatives of the introgression donor are included in the test instead. Similarly, when introgression occurred between two ancestral species, all of the derived species may inherit signals of that past gene flow. Thus, a single pair of introgressing species may leave signals of introgression in many more species pairs. To identify from such sets of signals the one species pair responsible for these, yet another statistic, called "*f*-branch", has been developed ([Malinsky et al. 2018](https://doi.org/10.1038/s41559-018-0717-x)).

The *D*-statistic and its related statistics have also been used to identify the specific genomic loci that were exchanged. This is done through sliding-window scans along the genome, or by calculating these statistics for particular short genomic regions. However, because the *D*-statistic itself has a large variance when applied to small genomic windows and because it is a poor estimator of the amount of introgression, additional statistics which are related to the *f*<sub>4</sub>-ratio have been designed specifically to investigate signatures of introgression in genomic windows along chromosomes. These statistics include *f*<sub>d</sub> ([Martin et al. 2015](https://doi.org/10.1093/molbev/msu269)), its extension *f*<sub>dM</sub> ([Malinsky et al. 2015](https://doi.org/10.1126/science.aac9927)), and the distance fraction *d*<sub>f</sub> ([Pfeifer and Kapan 2019](https://doi.org/10.1186/s12859-019-2747-z)). All of these statistics have recently been implemented in the program Dsuite ([Malinsky et al. 2021](https://doi.org/10.1111/1755-0998.13265)), which will be used in this turorial.
...
* **Dsuite:** The program [Dsuite]([Malinsky et al. 2021](https://doi.org/10.1111/1755-0998.13265)) calcuates the *D*-statistic and related statistics, and it does so very fast, allowing its application to a large number of species combinations. Dsuite is installed on lynx and can be called with `Dsuite` (note the capital "D").
...
* Place file `NC_031969.f5.sub1.vcf.gz`, which was already used in tutorial [Species-Tree Inference with SNP Data](../species_tree_inference_with_snp_data/README.md), in your current tutorial directory on lynx. If you don't have it anymore, download it from GitHub:
...
* Start the Dsuite analysis of the SNP dataset:
		...
		Dsuite Dtrios NC_031969.f5.sub1.vcf.gz individuals_dsuite.txt
...
1. The trio is assigned to P1, P2, and P3 so that P1 and P2 are the two species of the trio that share the largest number of derived sites, and that P2 and P3 share more derived sites with each other than P1 and P3 (*C*<sub>BBAA</sub> > *C*<sub>ABBA</sub> > *C*<sub>BABA</sub>). The idea behind this type of rearrangement is that the two species that share the largest number of derived sites are most likely the true two sister species in the trio, and thus rightfully placed in the positions P1 and P2. This assumption is expected to hold under certain conditions (e.g. clock-like evolution, absence of homoplasies, no or only weak introgression, and panmictic ancestral populations), but how reliable it is for real datasets is sometimes difficult to say. The output for this assignment is written to a file with the ending `_BBAA.txt`.
...
* **Question 3:** Differences between similar files can perhaps best be found with the `diff` command:
	...
	This should show a list of 124 lines that specify differences on individual lines or set of lines. For example, the first four lines of the output indicate that line 22 of `individuals_dsuite_BBAA.txt` differs from line 22 of file `individuals_dsuite_Dmin.txt` (the label `22c22` indicates that the change is on line 22):
		...
		22c22
		< altfas	neocan	telvit	0.784845	39.8722	2.3e-16	0.390878	12901.1	7185.56	866.188
		---
		> telvit	altfas	neocan	0.284543	20.8282	2.3e-16	0.962509	866.188	12901.1	7185.56
		...
