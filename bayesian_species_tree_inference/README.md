...
In this part of the tutorial, we are going to use the multi-species-coalescent model implementation of StarBeast3 ([Douglas et al. 2022](https://doi.org/10.1093/sysbio/syac010)) to estimate a time-calibrated species tree from a set of twelve alignments.
...
* Download the script `filter_genes_by_missing_data.rb` from GitHub to your tutorials directory on lynx:
	
		wget https://raw.githubusercontent.com/mmatschiner/phylogenomics/refs/heads/main/bayesian_species_tree_inference/scripts/filter_genes_by_missing_data.rb

	The first two command-line arguments required by this script are the names of a directory with sequence alignments and the name of another directory to which the filtered alignments should be written. In addition, a third arguments is required for the number of sequences per alignment that may be completely missing. In our case, this number should be 0 to ensure that all alignments contain at least partial sequence information for all species. Thus, execute the script using the following command:

		ruby filter_genes_by_missing_data.rb 72_genes 72_genes_red 0
...
* Because the script `convert.py` was already used in the [Bayesian Phylogenetic  Inference](../bayesian_phylogeny_inference/README.md) tutorial, you probably have this script already. If you can't find it, download it from GitHub:

		wget https://raw.githubusercontent.com/mmatschiner/phylogenomics/refs/heads/main/bayesian_species_tree_inference/scripts/convert.py

* Open a new file named `convert_alignments.sh` with a text editor available on lynx, such as Emacs:
...
* Write the following content to the new file:
		...
		do
			...
			python3 convert.py ${fasta} 72_genes_red/${gene_id}.nex -f nexus
		done

* Execute this new script with `bash`:

		bash convert_alignments.sh

* Remove the versions of the alignments in Fasta format:

		rm 72_genes_red/*.fasta

* Download the directory `72_genes_red` from lynx to your local computer, using `scp` (you'll need the `-r` option to download a directory).

* Open BEAUti. Even if you already installed the StarBeast3 package, you still need to load the BEAUti templates provided by StarBeast3. To do so, hover with the mouse over "Template" in BEAUti's "File" menu, and then click on "StarBeast3", as shown in the next screenshot.<p align="center"><img src="img/beauti1.png" alt="BEAUti" width="700"></p>In contrast to the first version of StarBEAST (\*BEAST), rate variation is modelled in StarBeast3 as a species trait that applies to all genes instead of being estimated individually for each gene; this is one of the reasons why StarBeast3 is much faster than the original implementation of StarBEAST (\*BEAST). The different clock models and their implementations are described in detail in [Ogilvie et al. (2017)](https://doi.org/10.1093/molbev/msx126).
...
* Continue to the "Site Model" tab. To reduce the computational demand of the analysis, select the relatively parameter-poor "HKY" model from the drop-down menu to the right of "Subst Model" instead of the GTR model. Make sure to set a tick in the checkbox to the right of "Kappa" to estimate the transition-transversion ratio. Also select "Estimated" from the drop-down menu to the right of "Frequencies" to estimate the base frequencies. The substitution rates will not be estimated through the site model this time; this will be done with the gene clock model (see below). The BEAUti window should then look as shown in the next screenshot.<p align="center"><img src="img/beauti6.png" alt="BEAUti" width="700"></p>
...
* Repeat the above step also for the names of the log files of all other gene trees.

* Then, save the file using "Save As" in BEAUti's "File" menu and name it "starbeast.xml".

* Copy file `starbeast.xml` to your tutorial directory on lynx using `scp`.

* On lynx, write a Slurm script named `run_starbeast.slurm` with the following content to submit the StarBeast3 analysis:
		...
		# Run beast2.
		beast2 starbeast.xml


* Install the StarBeast3 add-on package on lynx with the command-line version of BEAST2's PackageManager:

		packagemanager -add starbeast3

* Submit the StarBeast3 analysis on lynx with `sbatch`:

		sbatch run_starbeast.slurm
 
This analysis should take around 10 minutes. Instead of waiting for the analysis to finish, you could already continue with the next part of the tutorial.
...
* Continue to the "MCMC" tab. Name the log output file "concatenated.log", and specify "10000" as the log frequency next to "Log Every", as shown in the next screenshot.<p align="center"><img src="img/beauti23.png" alt="BEAUti" width="700"></p>

* Finally, set the name of the tree file to "concatenated.trees", and again specify "10000" as the log frequency, as shown below.<p align="center"><img src="img/beauti24.png" alt="BEAUti" width="700"></p>

* Then, use "Save As" in BEAUti's "File" menu to save the settings to a file named `concatenated.xml`.
 
* Copy file `concatenated.xml` to your working directory on lynx using `scp`.

* On lynx, copy the Slurm script `run_starbeast.slurm` to a new file named `run_concatenated.slurm`:

		cp run_starbeast.slurm run_concatenated.slurm
		
* Open the file `run_concatenated.slurm` in a text editor available on lynx, and replace `starbeast` with `concatenated` on lines 4, 14, and 17.
...
* Have a look at the estimates for the parameters named "Tree.t:Species.height" and "Tree.t:ENSDAR...height". These are the mean age estimates for the roots of the species tree and all gene trees.
...
* Next, use TreeAnnotator on lynx to generate MCC summary trees for the species tree of the analysis with the multi-species-coalescent model (file `starbeast_species.trees`) and for the tree based on concatenation (file `concatenated.trees`). Select first `starbeast_species.trees` and then `concatenated.trees` as input tree file and name the output file accordingly `starbeast_species.tre` or `concatenated.tre`. Apply a burnin percentage of 10 and specify that node ages in the summary tree should be according to mean age estimates:

		treeannotator -burnin 10 -height mean starbeast_species.trees starbeast_species.tre
		treeannotator -burnin 10 -height mean concatenated.trees concatenated.tre
...
* Open a new file named `summarize_gene_trees.sh` on lynx, and write the following content to it:
...
* To use commands from the ape R library, we'll need to write a short R script on lynx that should be named `convert_to_newick.r` and have the following content:
...
* Execute the script `convert_to_newick.sh` with `bash`:

		bash convert_to_newick.sh
...
* Download the MCC trees for the genes ENSDARG00000058676 and ENSDARG00000062267 (files `starbeast_ENSDARG00000058676.tre` and `starbeast_ENSDARG00000062267.tre` from lynx to your local computer using `scp`.
